...
async function runAgentForSection(section: string, customTopic?: string, wordCount: number = 800, writingStyle: string = 'standard') {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { section, topic, wordCount = 800, writingStyle = 'standard' } = body;
    ...
    console.log(`Running ${section} agent${topic ? ` for topic: ${topic}` : ''} with ~${wordCount} words in style: ${writingStyle}...`);

    // Run the agent
    const articles = await runAgentForSection(section, topic, wordCount, writingStyle);
