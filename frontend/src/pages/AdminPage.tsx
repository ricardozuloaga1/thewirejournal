...
import { 
  Zap, Loader2, Send, Trash2, Image, Check, X, Star,
  Sparkles, Scissors, Expand, Target, MessageSquare, BarChart3, Quote,
  Scale, FileText, Flame, Brain, Users, Edit3, ExternalLink
} from 'lucide-react'
import { WRITING_STYLES } from '../lib/writing-styles'
import { markdownToHtml } from '../lib/markdown'
import { articlesApi, agentsApi, imagesApi } from '../services/api'
...
// Satire section is separate from serious news
const SATIRE_SECTION = { id: 'satire', label: '🧅 The Lighter Side', isSatire: true }
...
export default function AdminPage() {
  ...
  async function loadArticles() {
    ...
    try {
      const drafts = await articlesApi.getDrafts()
      setArticles(drafts || [])
      
      const leadId = await articlesApi.getLeadArticleId()
      setLeadArticleId(leadId)
  ...
  async function runAllAgents() {
    ...
    try {
      const data = await agentsApi.runAllAgents(parseInt(wordCount), writingStyle)
      if (data.success) {
        setStatus(`Generated ${data.articles_created || data.articlesCreated} articles`)
        loadArticles()
      } else {
  ...
  // Open topic selection modal for a section
  function openTopicModal(section: string) {
    setPendingSection(section)
    setTopicInput('')
    setShowTopicModal(true)
  }

  // Run section agent with optional topic
  async function runSectionAgent(section: string, topic?: string) {
    setShowTopicModal(false)
    setPendingSection(null)
    setRunningSection(section)
    setError(null)
    setStatus(`Running ${section} agent${topic ? ` for "${topic}"` : ' (agent choosing topic)'}...`)
    try {
      const data = await agentsApi.runAgent(section, parseInt(wordCount), writingStyle, topic)
      if (data.success) {
        setStatus(`Generated ${data.articles_created || data.articlesCreated || 1} article(s)`)
        setTopicInput('')
        loadArticles()
      } else {
        setError(data.message || data.error)
  ...
  // Handle generate with user-provided topic
  function handleGenerateWithTopic() {
    if (pendingSection && topicInput.trim()) {
      runSectionAgent(pendingSection, topicInput.trim())
    }
  }

  // Handle let agent choose topic
  function handleAgentChoosesTopic() {
    if (pendingSection) {
      runSectionAgent(pendingSection)
    }
  }
  ...
  async function performEdit(action: string, prompt?: string) {
    ...
    try {
      const data = await articlesApi.editArticleWithAI(selectedArticle.id, action, prompt)
      
      if (data.success) {
        setSelectedArticle(data.article)
        setArticles(prev => prev.map(a => a.id === data.article.id ? data.article : a))
        setStatus(`Edit applied: ${data.changes?.join(', ') || 'article'} updated`)
  ...
  async function saveInlineEdit(field: 'title' | 'excerpt') {
    ...
    try {
      await articlesApi.updateArticle(selectedArticle.id, { [field]: value })
  ...
  async function generateImages() {
    ...
    try {
      const data = await imagesApi.generateImages(
  ...
  async function saveSelectedImage() {
    ...
    try {
      // Include caption in the image data
      const imageDataWithCaption = {
        ...selectedImage,
        caption: imageCaption.trim(),
      }
      await imagesApi.saveImage(selectedArticle.id, imageDataWithCaption as unknown as Record<string, unknown>)
  ...
  async function discardArticle(id: string) {
    ...
    try {
      await articlesApi.updateArticle(id, { status: 'discarded' })
      setArticles(prev => prev.filter(a => a.id !== id))
      setSelectedArticle(null)
  ...
  async function setAsLeadStory(articleId: string) {
    try {
      await articlesApi.setAsLeadStory(articleId)
  ...
  return (
    <div className="min-h-screen bg-[#FBF9F6]">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="bg-white border border-gray-200 p-4 mb-6">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {/* Satire Section - Separate from serious news */}
            <div className="w-full mt-2 pt-2 border-t border-gray-200">
              <button
                onClick={() => openTopicModal(SATIRE_SECTION.id)}
                disabled={isAnyAgentRunning}
                className={`px-4 py-2 text-sm font-bold border transition ${
                  runningSection === SATIRE_SECTION.id 
                    ? 'border-amber-600 bg-amber-500 text-white' 
                    : 'border-amber-300 bg-amber-50 text-amber-800 hover:border-amber-500 hover:bg-amber-100'
                } disabled:opacity-50`}
              >
                {runningSection === SATIRE_SECTION.id && <Loader2 className="w-3 h-3 animate-spin inline mr-2" />}
                {SATIRE_SECTION.label}
              </button>
              <span className="ml-2 text-xs text-amber-600 italic">Satirical · Not Real News</span>
            </div>
          </div>
          
          {/* Legacy Custom Topic - kept for direct input option */}
          <div className="flex gap-2">
            ...
            >
              ...
              <option value={SATIRE_SECTION.id}>{SATIRE_SECTION.label}</option>
            </select>
            <button
                    ...
                    <button
                      ...
                      className={`w-full text-left p-4 transition ${
                        selectedArticle?.id === article.id 
                          ? 'bg-amber-50 border-l-4 border-black' 
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`text-[10px] font-bold uppercase ${
                          article.section === 'satire' ? 'text-amber-600' : 'text-gray-400'
                        }`}>
                          {article.section === 'satire' ? '🧅 Satire' : article.section}
                        </span>
      ...
      {/* Image Modal */}
      {showImageModal && (
        <ImagePreviewModal showImageModal={showImageModal} setShowImageModal={setShowImageModal} generatedImages={generatedImages} setSelectedImageIndex={setSelectedImageIndex} selectedImageIndex={selectedImageIndex} imageCaption={imageCaption} setImageCaption={setImageCaption} saveSelectedImage={saveSelectedImage} />
      )}

      {/* Topic Selection Modal */}
      {showTopicModal && pendingSection && (
        <TopicSelectionModal
          pendingSection={pendingSection}
          sections={[...SECTIONS, SATIRE_SECTION]}
          ...
          onAgentChoosesTopic={handleAgentChoosesTopic}
          onGenerateWithTopic={handleGenerateWithTopic}
        />
      )}
    </div>
  )
